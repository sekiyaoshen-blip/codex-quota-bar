...
- 菜单栏同时显示 OpenAI 与阿里百炼的剩余额度，并显示 OpenAI 剩余重置次数；下拉菜单提供两方额度及重置时间详情。
- 可在 OpenAI 官方、DeepSeek 官方和阿里百炼 Token Plan 之间切换；切换完成后自动重启正在运行的 Codex。
- 使用阿里百炼时显示 Token Plan 一周额度及重置时间，每 5 分钟自动刷新。
