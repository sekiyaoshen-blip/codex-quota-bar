...
GPT 渠道之间保留原 GPT 模型；切到 DeepSeek 官方保留其兼容的 Pro/Flash 型号，否则使用 Pro；百炼使用 `deepseek-v4-pro-0813`，DeepSeek 转 GPT 使用 `gpt-6-astra`。型号改变时推理强度设为 `high`。迁移更新会话渠道与模型元数据，保留历史消息正文和更新时间，并修正分页历史的字节索引。代码依赖当前 Codex 本地数据库格式，无法识别的记录会使整个切换失败；不保证目标渠道支持所有保留的 GPT 型号。捕获到的失败会回滚，但跨文件与数据库操作不具备断电原子性，请勿在切换过程中强制终止额度栏。
